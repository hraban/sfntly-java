...
public class FontInfoMain {
  ...
  private static DataDisplayTable safeListNameEntries(Font font) {
    return FontInfo.listNameEntries(font);
  }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < fonts.length; i++) {
      ...
      try {
        ...
        System.err.println("Error listing name table entries: " + e.getMessage());
        e.printStackTrace();
      }
      ...
      try {
        if (options.general || options.all) {
          if (options.csv) {
            ...
            System.out.println(
                    prependDataAndBuildCsv(safeListNameEntries(font).csvStringArray(), fileName, i));
            System.out.println();
          } else {
            System.out.println(String.format("sfnt version: %s", FontInfo.sfntVersion(font)));
            System.out.println();
            ...
            System.out.println();
            System.out.println("Name Table Entries:");
            safeListNameEntries(font).prettyPrint();
            System.out.println();
          }
        }
      } catch (RuntimeException e) {
        System.err.println(String.format("Error reading font %d: %s", i, e.getMessage()));
        e.printStackTrace();
      }
